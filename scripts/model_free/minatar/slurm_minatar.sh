...
loss_functions=$4
detach_enc_critic=$5
checkpoint_dir=${6-"/checkpoint/voelcker/$SLURM_JOB_ID"}

echo "sbatch $SLURM_SUBMIT_DIR/slurm_minatar.sh $name $task_name $num_envs $loss_functions $detach_enc_critic $checkpoint_dir" > $checkpoint_dir/restart.sh
